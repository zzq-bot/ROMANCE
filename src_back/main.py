...
import yaml
from my_run import REGISTRY as myrunREGISTRY
import warnings
...
def my_main(_run, _config, _log):
    ...
    myrunREGISTRY[config["run_file"]](_run, config, _log)
...
if __name__ == '__main__':
    ...
    if config_dict["run_file"] == "robust":
        if config_dict['evaluate']:
            results_path = os.path.join(results_path, 'evaluate')
        results_save_dir = os.path.join(
            results_path, "romance", env, map_name,
            config_dict['name'] + config_dict['remark'],
            unique_token
        )
    elif config_dict["run_file"] == "collect":
        if config_dict['evaluate']:
            results_path = os.path.join(results_path, 'evaluate')
        
        results_save_dir = os.path.join(
            results_path, "collect", env, map_name,
