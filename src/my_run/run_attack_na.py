...
def run_sequential(args, logger):
    ...
    # no start evaluation here as it can be done in run_attack.py
    start_gen = 0
    ...
    for gen in range(start_gen, args.generation):
        ...
        last_gen_path = os.path.join(args.local_results_path, "last_save",
            ...
            if gen == start_gen and train_step == 0:
                for attacker_id, attacker in enumerate(population.attackers):
                    ...
                    for episode_idx in range(args.init_store_episode):
                        ...
                        episode_batch, mixed_points, attack_cnt, epi_return, _ = runner.run(test_mode=True,
                                                                                            gen_mask=gen_mask)
        ...
        # update archive behaviors if ego-agents change

        if (gen + 1) % args.save_archive_interval == 0:
            save_path = os.path.join(args.local_results_path, "attacker_population",
